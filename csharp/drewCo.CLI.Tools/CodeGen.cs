...
public class CodeGen
{
  ...
  public void OutputCSharp(CommandDef fromDef, CodeFile file, string? useNamespace = null)
  {
    ...
    {
      string typeName = ConvertTypeName(op);
  ...
  // --------------------------------------------------------------------------------------------------------------------------
  public static string ConvertTypeName(Type t)
  {
    string res = t.Name;
    if (t.IsArray)
    {
  ...
  // --------------------------------------------------------------------------------------------------------------------------
  public static string ConvertTypeName(CommandOption op)
  {
    if (!string.IsNullOrWhiteSpace(op.TypeHint))
    {
      var hint = new TypeHint(op.TypeHint);

      // NOTE: This is C# specific code!
      string res = null;
      if (hint.IsFloat)
      {
        switch (hint.DataSize)
        {
          case 16:
            res = "Half";
            break;
          ...
          case 64:
            ...
            break;
          default:
            throw new ArgumentOutOfRangeException("unsupported data size!");
        }
      }
      else
      {
        ...
        if (hint.DataSize == 8)
        {
          res = hint.IsSigned ? "sbyte" : "byte";
        }
        else
        {
          res = $"{(hint.IsSigned ? string.Empty : "U")}Int{hint.DataSize}";
        }
      }
  ...
  private void EmitConfigureFunction(CommandDef def, CodeFile file)
  {
    ...
    foreach (var op in def.Options)
    {
      ...
      file.WriteLine($"{opName}.DataType = typeof({ConvertTypeName(op)});");
